...
namespace DPO.Domain
{
    public class HtmlServices : BaseServices
    {
        ...
        public DropDownModel DropDownModelStrategicTypes(int? selectedId)
        {
            var result = Db.StrategicTypes.AsNoTracking().OrderBy(c => c.Name)
                .Select(i => new DropDownItem { ValueLong = (long)i.StrategicTypeId, Text = i.Name });

            return DropDown(result, selectedId);
        }

        public DropDownModel DropDownModelStrategicTypes(List<long> selectedIds)
        {
            var result = Db.StrategicTypes.AsNoTracking().OrderBy(c => c.Name)
                .Select(i => new DropDownItem { ValueLong = (long)i.StrategicTypeId, Text = i.Name });

            return DropDown(result, selectedIds);
        }
        ...
        public DropDownModel DropDownModelProjectDarComStatusTypes(UserSessionModel admin, ProjectDarComStatusTypeEnum? selectedId)
        {
            ...
            var hasCommission = admin.HasAccess(SystemAccessEnum.RequestCommission) ||
                   admin.HasAccess(SystemAccessEnum.ApproveCommissionRequests) ||
        ...
        private DropDownModel DropDown(IEnumerable<DropDownItem> query, List<long> selectedIds, bool noCache = false)
        {
            selectedIds = selectedIds ?? new List<long>();
            ...
            var items = ddItems.Select(i => new SelectListItemExt
            {
                ...
                Text = i.Text,
                DataRequirementLevel = i.RequirementLevel.HasValue ? i.RequirementLevel.ToString() : String.Empty,
                Selected = selectedIds.Contains(i.ValueLong),
                Disabled = i.Disabled
            }).ToList();
