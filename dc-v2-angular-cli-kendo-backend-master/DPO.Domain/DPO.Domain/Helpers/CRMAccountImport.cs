        ...
        public void InsertNewRecordsIntoDC(List<Account> newRecordList)
        {
            ...
            foreach (var newRecord in newRecordList)
            {
                ...
                if (newRecord.Address1Line1 != null)
                {
                    ...
                    dpoBusiness.Address.AddressLine1 = CheckStringLength(newRecord.Address1Line2, 50);
            ...
            //// Could try this to persist in db all at once, but what about the errors of individual recors? -- anand 

            //try
            //{
            //    Db.Context.Businesses.AddRange(businessList);
            //    Db.Context.SaveChanges();

            //    Console.WriteLine($"Completed insert of {businessList.Count} new records into DC");
            //}
            //catch (Exception ex)
            //{
            //    var errorMessage = $"Exception occurred while inserting new records due to {ex.InnerException?.Message}";
            //   LogAndSendExceptionMessage(errorMessage, ex);
            //}
        }
