...
namespace DPO.Data
{
    ...
    public partial class Repository
    {
        ...
        public IQueryable<User> QueryUsersInGroup(UserSessionModel user) {
            IQueryable<User> query;

            query = from u in this.Users
                    where u.GroupId == user.GroupId
                    select u;
        ...
        // Get all users linked to groups under the user
        //################################################################
        public IQueryable<User> QueryUsersInGroupAndBelowByUser(UserSessionModel user, bool includeUnallocated)
        {
            IQueryable<User> query = null;
            ...
            else
            {
                ...
                //include users in current group
                query = (from u in query select u)
                            .Union
                            (from u in QueryUsersInGroup(user) select u);
            }
        ...
        public IQueryable<User> QueryUserInGroupAndBelowByUserId(UserSessionModel admin, long? userId)
        {
            // Request their own record
            if (admin.UserId == userId)
            {
            ...
            {
                return this.QueryUsersInGroupAndBelowByUser(admin, true).Where(u => u.UserId == userId);
            }
