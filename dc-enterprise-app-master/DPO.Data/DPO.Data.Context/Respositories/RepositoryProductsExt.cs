...
namespace DPO.Data
{
    ...
    public partial class Repository
    {
        ...
        public IQueryable<Product> SellableProductByProductNumbers(UserSessionModel admin, string[] productNumbers)
        {
            return GetSellableProducts(admin).Where(p => productNumbers.Contains(p.ProductNumber)).Select(p => p);
        }
            ...
            else
            {
                result = from product in this.Products
                         ...
                         where product.ProductStatusId == (int)ProductStatusTypeEnum.Active
                            || product.ProductStatusId == (int)ProductStatusTypeEnum.Obsolete
                         select product;
            }
        ...
        //Same as GetProducts, but also include "HiddenSellable" Products
        public IQueryable<Product> GetSellableProducts(UserSessionModel admin)
        {
            IQueryable<Product> result;
            if (admin.UserTypeId >= UserTypeEnum.DaikinSuperUser)
            {
            ...
            else
            {
                result = from product in this.Products
                         ...
                         where product.ProductStatusId == (int)ProductStatusTypeEnum.Active
                            || product.ProductStatusId == (int)ProductStatusTypeEnum.Obsolete
                            || product.ProductStatusId == (int)ProductStatusTypeEnum.HiddenSellable
                         select product;
            }
        ...
        public IQueryable<Product> GetSellableProductsByProductNumber(UserSessionModel user, string productNumber)
        {
            var result = from products in GetSellableProducts(user) where products.ProductNumber == productNumber select products;
            return result;
        }
        ...
        public IQueryable<Product> GetSellableProductsByProductNumbers(UserSessionModel user, string[] productNumbers)
        {
            var result = from products in GetSellableProducts(user)
                         where productNumbers.Contains(products.ProductNumber)
                         select products;
            return result;
        }
        ...
        public IQueryable<ProductStatus> GetProductStatuses(UserSessionModel user)
        {
            ...
            if (user.UserTypeId >= UserTypeEnum.DaikinEmployee)
            {
            ...
            else
            {
                result = from productStatus in this.Context.ProductStatuses
                         where productStatus.ProductStatusId != (int)ProductStatusTypeEnum.New
                                ...
                                && productStatus.ProductStatusId != (int)ProductStatusTypeEnum.HiddenSellable
                                && productStatus.ProductStatusId != (int)ProductStatusTypeEnum.Inactive
                         select productStatus;
            }
        ...
        */
        private IQueryable<Product> FilterV2(IQueryable<Product> query, SearchProduct search)
        {
            ...
            if (search.ProductNumbers.Count > 0) {
                query = query.Where(s => search.ProductNumbers.Contains(s.ProductNumber));
            }
