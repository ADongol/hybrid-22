...
namespace DPO.Data
{
    ...
    public partial class Repository
    {
        ...
        public IQueryable<CommissionRequest> CommissionRequests
        {
            get { return this.GetDbSet<CommissionRequest>(); }
        }

        //public IQueryable<CommissionCalculation> QueryCommissionCalculationViewable(UserSessionModel user)
        //{
        //    IQueryable<CommissionCalculation> query;

        //    query = from commisionCalculation in this.Context.CommissionCalculations
        //            join project in QueryProjectsViewableByUser(user)
        //            on commisionCalculation.ProjectId equals project.ProjectId
        //            select commisionCalculation;
        ...
        public IQueryable<CommissionRequest> QueryCommissionRequestsViewable(UserSessionModel user)
        {
            ...
            query = from commision in this.Context.CommissionRequests
                    join project in QueryProjectsViewableByUser(user)
                    on commision.ProjectId equals project.ProjectId
                    select commision;
            
            return query;
        }

        //public IQueryable<CommissionRequest> QueryCommissionRequests(UserSessionModel user)
        //{
        //    IQueryable<CommissionRequest> query;
        //    return query;          
        //}

        //public CommissionCalculation CommissionCalculationCreate(long projectId, long quoteId)
        //{
        //    var entity = new CommissionCalculation();
        //    entity.CommissionCalculationId = this.Context.GenerateNextLongId();
           
        //    entity.ProjectId = projectId;

        //    entity.QuoteId = quoteId;

        //    this.Context.CommissionCalculations.Add(entity);
        ...
        public IQueryable<CommissionRequest> QueryCommissionRequestsViewableBySearch(UserSessionModel user, SearchCommissionRequests search)
        {
            ...
            if (search != null && search.ReturnTotals)
            {
                try
                {
                    ...
                    //search.TotalRecords = query.AsEnumerable().Count();
                }
                catch (Exception ex)
                {
                    throw ex;
