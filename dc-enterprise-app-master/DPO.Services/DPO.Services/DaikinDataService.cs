    ...
    class DaikinDataService
    {
        static void Main(string[] args)
        {
            ...
            using (var data = new Repository())
            {
                ...
                if (args.Length == 1)
                {
                    ...
                    if (string.Compare(args[0], "RecalculateQuotePrices", true) == 0)
                    {
        ...
        static void DaikinImport(Dictionary<string, Product> currentProductsLookup)
        {
            ...
            using (var service = new DaikinServices())
            {
                ...
                var fromdate = DateTime.Today.AddDays(-999);
        ...
        //This function recalculate price for all quotes in all Open Projects
        static void RecalculateQuotePrices()
        {
            ILog Log = LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
            //Get superUser's userSession
            var email = "daikincity@daikincomfort.com";

            AccountServices accountService = new AccountServices();
            ServiceResponse resp = accountService.GetUserSessionModel(email);
            var admin = resp.Model as UserSessionModel;

            QuoteServices quoteSvc = new QuoteServices();
            using (var Db = new Repository())
            {
                //Get all Quotes in open Projects
                var quotesQuery = from q in Db.Quotes
                                  join p in Db.Projects on q.ProjectId equals p.ProjectId
                                  where p.ProjectStatusTypeId == ProjectStatusTypeEnum.Open
                                    && q.RecalculationRequired == true
                                  select q;
                ...
                foreach (var q in quotes)
                {
                    try
                    {
                        ...
                        quoteSvc.Response = new ServiceResponse();

                        //Get QuoteModel
                        QuoteModel quoteModel = quoteSvc.GetQuoteModel(admin, q.ProjectId, q.QuoteId).Model as QuoteModel;
                        Console.WriteLine("Get Quote successfully-QuoteId:" + q.QuoteId);

                        //Recalculate quote price.
                        Console.WriteLine("Recalculate quote " + q.QuoteId + " started.");
                        quoteSvc.QuoteRecalculate(admin, quoteModel);
                        Console.WriteLine("Recalculate quote " + q.QuoteId + " finished.");
                        successCount++;
                        //Log.Info("Recalculated quote " + q.QuoteId + " finished.");
                    ...
                    catch (Exception e)
                    {
                        ...
                        Console.WriteLine("Error: GetQuoteModel Failed - QuoteId: " + q.QuoteId);
                        Console.WriteLine("Error Message: " + e.Message);
                        Console.WriteLine("================================================");

                        Log.Error("GetQuoteModel Failed - QuoteId: " + q.QuoteId);
                ...
                Log.Info("*****Finished Recalculate Quotes*****");
