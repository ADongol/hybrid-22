...
export class ProjectGridComponent implements OnInit, AfterContentInit, AfterViewInit {
    ...
    constructor(private toastrSvc: ToastrService, private userSvc: UserService,
        private systemAccessEnum: SystemAccessEnum, private http: Http,
    ...
    public SaveGridSettings() {
        ...
        localStorage["projects-grid-settings"] = gridsettings;
    }

    public LoadGridSettings() {
        var grid = jQuery("#project-grid").data("kendoGrid");
        var baseSettings = this.getBaseGridOptions();
        var savedSettings = JSON.parse(localStorage["projects-grid-settings"]);
    ...
    public CombinedGridSettings() {
        var baseSettings = this.getBaseGridOptions();
        //return baseSettings;
        if (localStorage.getItem("projects-grid-settings") === null) {
            return baseSettings;
        } else {
            var savedSettings = JSON.parse(localStorage["projects-grid-settings"]);
            var gridSettings = this.MergeGridSettings(baseSettings, savedSettings);
