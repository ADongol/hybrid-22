...
angular.module("DPO.Projects").controller('orderFormController', ['$scope', '$log', '$resource', 'quoteService', 'orderService', 'DPOprojectService', 'addressService', function ($scope, $log, $resource, quoteService, orderService, DPOprojectService, addressService, $templateCache) {
        ...
        if (result.isok) {
            ...
            $scope.productGridOptions = {
                ...
                columns: [{
                    ...
                    title: "Product",
                    width: "15%",
                    template: "#if(lineItemTypeId==2){# <span>#=codeString#</span> #} else{ # <span>#=productNumber#</span> #}#"
                }, {
                    field: "description",
                    title: "Product Description",
                    width: "32%",
                    template: "#if(lineItemTypeId==2){# <span>Configured Product</span> #} else{ # <span>#=description#</span> #}#"
                }, {
                    field: "quantity",
                    title: "Qty",
                    width: "5%"
    ...
    $scope.SubmitOrder = function () {
        //ShipToName is needed for order submittal xml
        $scope.OrderVM.shipToName = $scope.projectVM.shipToName;
                ...
                orderService.postOrder($scope.OrderVM).$promise.then(function (result) { // api controller 
                    if (result.isok) {
                        ...
                        bootbox.alert("<p>Thank you for submitting the order. Your Daikin Customer Service Representative will review the order and get back to you within 24 hours.<br/> <br/>To cancel the order, please contact your Daikin Customer Service Representative.</p>", function () {
                            ...
                            window.location.replace("/v3/#/quote/" + $scope.OrderVM.quoteIdStr + "/existingRecord");
                        });
    ...
    $scope.onUploadSuccess = function (e) {
        ...
        var filenames = $(".k-filename");

        for (var i = 0; i < filenames.length; i++) {
            $(filenames[i]).html("<a href='/ProjectDashboard/GetPOAttachment?quoteId=" + $scope.OrderVM.quoteIdStr + "&poAttachmentFileName=" + $scope.OrderVM.poAttachmentFileName + "' target='_blank'>" + $scope.OrderVM.poAttachmentFileName + "</a>");
        }
    }
