...
namespace DPO.Web.Controllers
{
    public class CityCMSController : BaseController
    {
        ...
        [HttpGet]
        [Authorise(Accesses = new[] { SystemAccessEnum.ViewMaintenanceInfo })]
        public ActionResult MaintenanceConfig()
        {
            var model = services.GetMaintenanceConfigs();
            return View(model);
        }

        [HttpGet]
        public ActionResult MaintenanceConfigEdit(int configId)
        {
            return View(services.GetMaintenanceConfigEdit(configId));
        }

        [HttpGet]
        public ActionResult MaintenanceConfigDelete(int configId)
        {
            services.GetMaintenanceConfigDelete(configId);
            return RedirectToAction("MaintenanceConfig");
        }

        [HttpPost]
        public ActionResult MaintenanceConfig(MaintenancePageModel model)
        {
            if (ModelState.IsValid)
            {
                services.SaveMaintenaceConfig(model);
                return RedirectToAction("MaintenanceConfig");
            }
            return View(model);
        }
        ...
        [HttpPost]
        public ActionResult MaintenanceConfigEdit(MaintenancePageModel model)
        {
            if (ModelState.IsValid)
            {
                services.SaveMaintenaceConfig(model);
