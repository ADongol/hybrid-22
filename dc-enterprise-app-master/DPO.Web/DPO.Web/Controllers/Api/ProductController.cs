...
namespace DPO.Web.Controllers
{
    ...
    public class ProductController : BaseApiController
    {
        ...
        public HttpResponseMessage Products(long? quoteId = null)
        {
            ...
            response.Headers.Location = new Uri(baseUrl + "/v3/#/product");
            return response;
        }
        ...
        [HttpPost]
        public ServiceResponse GetProductsByProductNumbers(ProductNumberQueryList ProductNumberList)
        {
            if (ProductNumberList != null && ProductNumberList.ProductNumbers.Count > 0)
            {
                var queryModel = new ProductsQueryModel();
                queryModel.ProductNumbers = ProductNumberList.ProductNumbers;
                return productservices.GetAllProducts(queryModel);
            }
            return null;
        }
        ...
        public ServiceResponse AddProductToQuoteByProductNumber(ProductModel productModel)
        {
            ...
            ServiceResponse resp = productservices.GetSellableProductByProductNumber(this.CurrentUser, productModel);
            ...
            if (serviceResponse.IsOK || systemMatchupModel.ContinueAdding)
            {
                ...
                if (systemMatchupModel.ValidProducts.Count > 0)
                {
                    foreach (string productNumber in systemMatchupModel.ValidProducts)
                    {
                        ...
                        var productId = productservices.GetSellableProductId(this.CurrentUser, productNumber);
                        if (productId != 0)
                        {
