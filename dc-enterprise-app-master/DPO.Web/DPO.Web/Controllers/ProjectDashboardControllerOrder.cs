...
namespace DPO.Web.Controllers
{
    public partial class ProjectDashboardController
    {
        ...
        {
            var quoteId = orderVMLight.QuoteId;
            var projectId = orderVMLight.ProjectId;
            ...
            var _last5DigitsOfProjectId = orderVMLight.ProjectIdStr.Substring(orderVMLight.ProjectIdStr.Length - 5);
            var nameFile = "Daikin City Order " + DateTime.Now.ToString("MM-dd-yyyy") +
                               "-" + _last5DigitsOfProjectId + ".pdf";

            var subPath = grandParent + "/CustomerDataFiles/OrderSubmitFiles/" + orderVMLight.QuoteId;
            ...
            var filePath = grandParent + "/CustomerDataFiles/OrderSubmitFiles/" + orderVMLight.QuoteId + "/" + nameFile;
        ...
        public ActionResult SendEmailOrderSubmit(OrderViewModelLight orderVMLight)
        {
            ...
            if (orderVMLight.HasConfiguredModel)
            {
                ...
                emailModel.Subject = string.Format("BTO Order Submitted" + "-Business: " + orderVMLight.BusinessName + "-Project: " + orderVMLight.ProjectName);
            }
            else
            {
                ...
                emailModel.Subject = string.Format("Order Submitted" + "-Business: " + orderVMLight.BusinessName + "-Project: " + orderVMLight.ProjectName);
            }
            ...
            var admin = accountService.GetUserSessionModel(orderVMLight.CreatedByUserId).Model as UserSessionModel;
            var businessService = new BusinessServices();
            var businessModel = businessService.GetBusinessModel(admin, admin.BusinessId).Model as BusinessModel;
            ...
            if (orderVMLight.Project.OwnerId != CurrentUser.UserId)
            {
                var userService = new UserServices();
                var projectOwner = userService.FindUserInGroupAndBelowByUserId(CurrentUser, orderVMLight.Project.OwnerId).Model as UserModel;
            ...
            var OrderPdfFile = "Daikin City Order " + DateTime.Now.ToString("MM-dd-yyyy") +
                                  "-" + _last5DigitsOfProjectId + ".pdf";

            var OrderPdfFilePath = grandParent + "/CustomerDataFiles/OrderSubmitFiles/" + orderVMLight.QuoteId + "/" + OrderPdfFile;
 
            //TODO: Should be fixed
            var DARPdfFile = "Daikin City Discount Request " +
                                    DateTime.Now.ToString("MM-dd-yyyy") +
                                    "-" +
                                   _last5DigitsOfProjectId + ".pdf";

            var DARPdfFilePath = grandParent + Utilities.Config("dpo.setup.customerdata.location") 
                                        + "DiscountRequestFiles/" + orderVMLight.QuoteId + "/" + DARPdfFile;

           // log.DebugFormat("DarPdfFilePath: {0}", DARPdfFilePath);

            var COMPdfFile = orderVMLight.CommissionRequestId.ToString() + ".pdf";

            var COMPdfFilePath = grandParent + "/CustomerDataFiles/DiscountRequestFiles/" 
                                        + orderVMLight.QuoteId + "/" + COMPdfFile;
            ...
            if (orderVMLight.DiscountRequestId != null)
            {
                ...
                else
                {
                    ...
                    orderVMLight.DARAttachmentFileName = System.IO.Directory.GetFiles(grandParent + Utilities.Config("dpo.setup.customerdata.location") + "DiscountRequestFiles/" + orderVMLight.QuoteId + "/").FirstOrDefault();
                }
            }
            else
            {
           ...
           // log.DebugFormat("Project Date: {0}", emailModel.Order.ProjectDate);
            ...
            if (orderVMLight.IsExpedite)
            {
                emailModel.IsExpedite = true;
                emailModel.OrderExpediteReason = orderVMLight.OrderExpediteInfo.OrderExpediteReasons;
            }
            ...
            var additionalDocsDirectory = new System.IO.DirectoryInfo(Utilities.GetAdditionalOrderDocsDirectory(orderVMLight.QuoteId));
            var additionalDocFiles = (from f in additionalDocsDirectory.GetFiles()
                                  orderby f.LastWriteTime descending
                                  select f).ToList();
            ...
            if (additionalDocFiles != null && additionalDocFiles.Count() > 0)
            {
                foreach(var additionalDoc in additionalDocFiles)
                {
                    emailModel.OtherAttachmentFiles.Add(additionalDoc.FullName);
                }
            }
        ...
        public ActionResult SendEmailToTeamWhenFailToSendEmailOnOrder(long quoteId, string exception)
        {
            ...
            var projectModel = projectService.GetProjectModel(this.CurrentUser, projectId).Model as ProjectModel;
            ...
            var emailMessage = string.Format(@"An error happned when try to send email for Order Submit.Below is the Order Details:" + Environment.NewLine +
        ...
        {
            var userSerivce = new UserServices();
...
#pragma warning restore CS0472 // The result of the expression is always 'true' since a value of type 'decimal' is never equal to 'null' of type 'decimal?'
        ...
        public ActionResult OrderDelete(OrderViewModelLight model)
        {
            ...
            if (ProcessServiceResponse(this.ServiceResponse))
            {
                ...
                string url = "/v3/#/quote/" + model.QuoteId + "/existingRecord";
                return Redirect(url);
            }
        ...
        {
            var nameFile = poAttachmentFileName;
            var root = Server.MapPath("~");
            var parent = System.IO.Path.GetDirectoryName(root);
            var grandParent = System.IO.Path.GetDirectoryName(parent);
            var filePath = grandParent + "/CustomerDataFiles/POAttachment/" + quoteId + "/" + nameFile;
