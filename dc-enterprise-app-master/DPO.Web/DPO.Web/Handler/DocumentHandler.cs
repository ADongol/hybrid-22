...
namespace DPO.Web
{
    ...
    public class DocumentHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (string.Compare(this.type, "QuotePrint", true) == 0 || 
                string.Compare(this.type, "QuotePrintWithCostPrice", true) == 0 )
            {
            ...
            if (string.Compare(this.type, "DiscountRequestPrint", true) == 0 || 
                string.Compare(this.type, "DiscountRequestPrintWithCostPricing", true) == 0)
            {
            ...
            if (string.Compare(this.type, "CommissionRequestPrint", true) == 0 || 
                string.Compare(this.type, "CommissionRequestPrintWithCostPricing", true) == 0)
            {
            ...
            if(string.Compare(this.type, "OrderPrint", true) == 0 || 
                string.Compare(this.type, "OrderPrintWithCostPricing", true) == 0)
            {
                var query = context.Request.Url.Query as string;
            ...
            if (string.Compare(this.type, "QuoteOrder", true) == 0)
            {
                ...
                string poAttachment = Path.Combine(Utilities.GetPOAttachmentDirectory(quoteid), 
                                            context.Request["filename"] as string);

                if (new AccountServices().CanAccessQuote(HttpContext.Current.User.Identity.Name, quoteid) 
                    && File.Exists(poAttachment))
                {
            ...
            if (string.Compare(this.type, "AdditionalPODocs", true) == 0)
            {
                long quoteid = long.Parse(this.id);

                string additionalDocs = Path.Combine(Utilities.GetAdditionalOrderDocsDirectory(quoteid), 
                                        context.Request["filename"] as string);

                if (new AccountServices().CanAccessQuote(HttpContext.Current.User.Identity.Name, quoteid) 
                    && File.Exists(additionalDocs))
                {
                    context.Response.ContentType = MimeMapping.GetMimeMapping(additionalDocs);

                    context.Response.TransmitFile(additionalDocs);
                }
                return;
            }
