...
namespace DPO.Services.Light
{
    public class OrderServiceLight : BaseServices, IOrderServiceLight
    {
        ...
        public DiscountRequestServices DARService = new DiscountRequestServices();
        ...
        public ServiceResponse GetNewOrder(UserSessionModel user, long quoteId)
        {
            ...
            var query = from quote in this.Context.Quotes
                        ...
                        select new OrderViewModelLight
                        {
                            ...
                            //RequestedDiscountPercentage = quote.DiscountRequests.Where(d => d.DiscountRequestId == quote.DiscountRequestId)
                            //                            .FirstOrDefault().RequestedDiscount,
                            //ApprovedDiscountPercentage = (decimal)quote.DiscountRequests.Where(d => d.DiscountRequestId == quote.DiscountRequestId)
                            //                            .FirstOrDefault().ApprovedDiscount,
                            //RequestedDiscountPercentage = quote
            ...
            if (model.DiscountRequestId != null) {
                var discountRequestModel = DARService.GetDiscountRequestModel(user, new DiscountRequestModel { DiscountRequestId = model.DiscountRequestId, ProjectId = model.ProjectId, QuoteId = model.QuoteId }).Model as DiscountRequestModel;
                if (discountRequestModel.DiscountRequestStatusTypeId == (byte?)DiscountRequestStatusTypeEnum.Approved) {
                    //TODO: remove dividing by 100
                    //model.RequestedDiscountPercentage = discountRequestModel.RequestedDiscount/100;
                    //model.ApprovedDiscountPercentage = discountRequestModel.ApprovedDiscount/100;

                    model.RequestedDiscountPercentage = discountRequestModel.RequestedDiscount;
                    model.ApprovedDiscountPercentage = discountRequestModel.ApprovedDiscount;
                }
            }
            ...
            if (existedOrder != null) // view 
            {
                var query = from order in this.Context.Orders
                            ...
                            select new OrderViewModelLight
                            {
                                ...
                                //POAttachmentFileName = poAttachmentFile.FileName,
                ...
                var poAttachmentFile = this.Context.OrderAttachments.Where(a => a.OrderId == existedOrder.OrderId
                                            && a.OrderAttachmentTypeId == 1).FirstOrDefault();

                var additionalDocs = this.Context.OrderAttachments.Where(a => a.OrderId == existedOrder.OrderId
