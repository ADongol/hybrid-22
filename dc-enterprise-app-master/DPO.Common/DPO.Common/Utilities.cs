...
namespace DPO.Common
{
    ...
    public partial class Utilities
    {
        ...
        public static Boolean RemoveUploadedFile(string fileToRemove, long quoteId)
        {
            var result = false;
           // if     ///(fileToRemove != null && fileToRemove.ContentLength > 0)
            //{
                //var fileName = Path.GetFileName(fileToRemove.FileName);

                var dirPathPOAttachment = GetPOAttachmentDirectory(quoteId);
                var dirPathAdditionalDocs = GetAdditionalOrderDocsDirectory(quoteId);

                var poFileToRemove = Path.Combine(dirPathPOAttachment, fileToRemove);
                var additionalDocToRemove = Path.Combine(dirPathAdditionalDocs, fileToRemove);

                if (poFileToRemove != null)
                {  //confirms user trying to delete PO Attachment

                    if (File.Exists(poFileToRemove))
                    {
                        File.Delete(poFileToRemove);
                        result = true;
                    }

                    if (File.Exists(additionalDocToRemove)) //confirms user trying to delete Additional Docs Attachment
                    {
                        File.Delete(additionalDocToRemove);
                        result = true;
                    }
               // }
            }
        ...
        {
            return Utilities.Config("dpo.setup.documentservice.url");
        ...
        public static string GetAdditionalOrderDocsDirectory(long quoteId)
        {
            var directory = GetDocumentDirectory() + Utilities.Config("dpo.setup.customerdata.location") + "AdditionalOrderDocuments\\" + quoteId.ToString() + "\\";
        ...
        public static string GetAdditionalDocsFileName(string quoteId)
        {
            return "DPO_AdditionalDocs_" + quoteId.ToString() + ".zip";
        }
        ...
        public static FileInfo[] GetAllFilesFromDirectory(string directoryPathName)
        {
            var dir = new DirectoryInfo(directoryPathName);
            return dir.GetFiles();
        }
