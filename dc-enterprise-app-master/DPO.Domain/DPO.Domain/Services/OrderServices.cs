    ...
    public partial class OrderServices : BaseServices, IOrderServices
    {
        ...
        public DPO.Model.Light.OrderSendEmailModel getOrderSendEmailModel(OrderViewModelLight orderVMLight)
        {
            ...
            return new DPO.Model.Light.OrderSendEmailModel
            {
                ...
                RequestedDiscountPercent = orderVMLight.RequestedDiscountPercentage,
                ...
                IsExpedite = orderVMLight.IsExpedite,
                OrderExpediteReason = orderVMLight.OrderExpediteInfo.OrderExpediteReasons
            };
        }
        #endregion Emails
        ...
        public ServiceResponse GetOrderModel(long orderId)
        {
            ...
            if (!string.IsNullOrEmpty(orderId.ToString()))
            {
                var query = from order in this.Context.Orders
                            ...
                            join business in this.Db.Businesses on owner.BusinessId equals business.BusinessId
                            //join ort in this.Context.OrderAttachments on order.OrderId equals ort.OrderId
                            join expedite in this.Context.OrderExpediteInfoes on order.OrderId equals expedite.OrderId into Lexpedite
                            from expedite in Lexpedite.DefaultIfEmpty()
                            ...
                            select new OrderViewModelLight
                            {
                                ...
                                //POAttachmentFileName = ort.FileName,
                                IsExpedite = (expedite == null) ? false : true,
                                OrderExpediteInfo = new OrderExpediteInfoViewModel
                                {
                                    OrderId = order.OrderId,
                                    OrderExpediteReasons = expedite.OrderExpediteReasons
                                },
                                Timestamp = (order.Timestamp != null) ? order.Timestamp : DateTime.Now
                            };
                ...
                var poAttachmentFile = this.Context.OrderAttachments.Where(a => a.OrderId == orderId
                                            && a.OrderAttachmentTypeId == 1).FirstOrDefault();

                var additionalDocs = this.Context.OrderAttachments.Where(a => a.OrderId == orderId
                                            && a.OrderAttachmentTypeId != 1).Select(y => y.FileName);

                model.POAttachmentFileName = poAttachmentFile.FileName;
                model.AdditionalDocsAttachment = (additionalDocs != null && additionalDocs.Count() > 0) ?
                                            additionalDocs.ToArray() : null;
            }
                ...
                !string.IsNullOrEmpty(quoteId.ToString()))
            {
                var query = from order in this.Db.QueryOrdersViewableByUser(user)
                            ...
                            select new OrderViewModel
                            {
                                ...
                                DeliveryContactPhone = order.DeliveryContactPhone,
                                //SubmittedByUserId = mod.UserId,
                                SubmittedByUser = order.User.FirstName + " " + order.User.LastName,
                                SubmittedByUserId = order.SubmittedByUserId,
                                SubmitDate = (order.SubmitDate != null) ? order.SubmitDate : DateTime.Now,
                                ProjectDate = (project.ProjectDate != null) ? project.ProjectDate : DateTime.Now,
                                ERPOrderDate = (order.ERPOrderDate != null) ? order.ERPOrderDate.Value : (System.DateTime?)null,
                                ERPInvoiceNumber = (order.ERPInvoiceNumber != null) ? order.ERPInvoiceNumber : null,
                ...
                var poAttachmentFile = this.Context.OrderAttachments.Where(a => a.OrderId == model.OrderId
                                            && a.OrderAttachmentTypeId == 1).FirstOrDefault();

                var additionalDocs = this.Context.OrderAttachments.Where(a => a.OrderId == model.OrderId
                                            && a.OrderAttachmentTypeId != 1).Select(y => y.FileName);
        ...
        public ServiceResponse GetOrderListViewModel(UserSessionModel user, OrderViewModel orderVM)
        {
            ...
            if (!string.IsNullOrEmpty(orderVM.QuoteId.ToString()))
            {
                ...
                {
                    var poAttachmentFile = this.Context.OrderAttachments.Where(a => a.OrderId == order.OrderId
                                            && a.OrderAttachmentTypeId == 1).FirstOrDefault();

                    var additionalDocs = this.Context.OrderAttachments.Where(a => a.OrderId == order.OrderId
                                                && a.OrderAttachmentTypeId != 1).Select(y => y.FileName);

                    order.POAttachmentFileName = poAttachmentFile.FileName;
                    order.AdditionalDocsAttachment = (additionalDocs != null && additionalDocs.Count() > 0) ?
                                                additionalDocs.ToArray() : null;
                }
            }
        ...
        public ServiceResponse GetOrdersForGrid(UserSessionModel user, SearchOrders search)
        {
            ...
            var query = from order in this.Db.QueryOrderViewableBySearch(user, search)// TODO: this QueryOrderViewableBySearch is very slow. It can cause time out error with super user
                        ...
                        select new OrderGridViewModel
                        {
                            ...
                            poAttachmentName = order.OrderAttachments
                                                    .FirstOrDefault(x=> x.OrderId == order.OrderId && x.OrderAttachmentTypeId == 1)
                                                    .FileName,
                            hasAdditionalDocs = order.OrderAttachments.Any(x=>x.OrderId == order.OrderId && x.OrderAttachmentTypeId ==2),
                            additionalDocFileName = (order.OrderAttachments.Any(x => x.OrderId == order.OrderId && x.OrderAttachmentTypeId == 2)) ?
                                                         "additionalDocs.zip" : string.Empty,
        ...
        public bool CheckOrderStatus(long quoteId)
        {
            var isOrderSubmitted = this.Context.Orders.Any(x => x.QuoteId == quoteId);
