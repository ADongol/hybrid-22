...
namespace DPO.Domain
{
    ...
    public partial class UserServices : BaseServices
    {
        ...
        {
            this.Response.Model = Db.CityAreas.Where(c => c.CityAreaId == 1 || c.CityAreaId == 3).Select(i => i.CityAreaId).ToList(); // Library Only
        ...
        public ServiceResponse FindUserInGroupAndBelowByUserId(UserSessionModel currenUser, long? userId)
        {
            return FindUserModelInGroupAndBelow(currenUser, new UserModel() { UserId = userId});
        }

        public ServiceResponse FindUserModelInGroupAndBelow(UserSessionModel user, UserModel templateModel, bool isEditing = false)
        {
            UserModel model = null;
            if (templateModel != null)
            {
                ...
                if (userId.HasValue)
                {
                    var query = from u in this.Db.QueryUserInGroupAndBelowByUserId(user, userId.Value)
                                select
                                new UserModel
                                {
                                    UserId = u.UserId,
        ...
        public ServiceResponse PostModel(UserSessionModel admin, UserModel model)
        {
            ...
            try
            {
                ...
                if (this.Response.IsOK)
                {
                    ...
                    //Moved this block to prevent user from their account being created if password validation fails
                    //user account gets persisted in ApplyBusinessRules method
                    if (model.IsRegistering)
                    {
                ...
                /////*** COMMENTED BY Anand because of redundant if statement - 'this.Response.IsOK'
                ////*****
                ////Moved this block to prevent user from their account being created if password validation fails
                ////user account gets persisted in ApplyBusinessRules method
                //if (this.Response.IsOK)
            ...
            if (entity.IsRegistering)
            {
                ...
                else// is New Business
                {
                    ...
                    if (model.Business.ParentBusinessId != 0 && 
                        model.Business.ParentBusinessId != 0 && 
                        entity.Business.BusinessId != 0)
                    {
