...
namespace DPO.Domain
{
    public partial class OrderServices : BaseServices
    {
        public void RulesOnValidateModel(OrderViewModelLight model)
        {
            ...
            if (model.IsExpedite == true && model.OrderExpediteInfo.OrderExpediteReasons == null)
            {
                this.Response.Messages.AddError("ExpediteReasons", ResourceUI.ExpediteReasonsRequired);
            }
                ...
                foreach (string chac in replaceCharacters)
                  {
                    ...
                    //validate the file names of invalid characters in additional docs array
                    if (model.AdditionalDocsAttachment != null && model.AdditionalDocsAttachment.Count() > 0)
                    {
                        foreach(string additionalDoc in model.AdditionalDocsAttachment)
                        {
                            additionalDoc.Replace(chac, "_");
                        }
                    }
