...
namespace DPO.Domain
{
    public partial class QuoteServices
    {
        ...
        public void RulesToCalculateQuoteTotals(UserSessionModel user, Quote quote, List<QuoteItem> items)
        {
            ...
            quote.DiscountPercentage = calc.TotalDiscountPercentage;
        ...
        public QuoteCalculationModel CalculateTotalDiscountsApproved(Quote quote)
        {
            ...
            var req = new QuoteCalculationRequest
            {
                //CommissionPercentage = quote.ApprovedCommissionPercentage,
                DiscountPercentage = quote.ApprovedDiscountPercentage,
                GrossMarginMarkUp = quote.GrossMarginMarkUp,
                IsGrossMargin = quote.IsGrossMargin,
                TotalList = quote.TotalList,
                TotalListSplit = quote.TotalListSplit,
                TotalListVRV = quote.TotalListVRV,
        ...
        //new
        public QuoteCalculationModel CalculateApprovedTotals(DiscountRequestModel discountRequest)
        {
            var totalNet = ((discountRequest.Quote.TotalNetCommission ?? 0) + (discountRequest.Quote.TotalNetNonCommission ?? 0));
            var _quote = new QuoteCalculateModel
            {
                GrossMarginMarkUp = discountRequest.GrossMarginMarkUp,

                DiscountPercentage = discountRequest.ApprovedDiscount,
                DiscountPercentageSplit = discountRequest.ApprovedDiscountSplit,
                DiscountPercentageVRV = discountRequest.ApprovedDiscountVRV,
                DiscountPercentageUnitary = discountRequest.ApprovedDiscountUnitary,
                DiscountPercentageLCPackage = discountRequest.ApprovedDiscountLCPackage,

                IsGrossMargin = discountRequest.Quote.IsGrossMargin,
                TotalList = discountRequest.Quote.TotalList ?? 0,
                TotalListSplit = discountRequest.Quote.TotalListSplit,
                TotalListVRV = discountRequest.Quote.TotalListVRV,
                TotalListUnitary = discountRequest.Quote.TotalListUnitary,
                TotalListLCPackage = discountRequest.Quote.TotalListLCPackage,
                TotalNet = totalNet,
                TotalNetSplit = discountRequest.Quote.TotalNetSplit,
                TotalNetVRV = discountRequest.Quote.TotalNetVRV,
                TotalNetUnitary = discountRequest.Quote.TotalNetUnitary,
                TotalNetLCPackage = discountRequest.Quote.TotalNetLCPackage
            };

            var approvedTotals = CalculateQuoteTotals(_quote);
        ...
        //new
        public QuoteCalculationModel CalculateStandardTotals(QuoteListModel quote) {
            var totalNet = ((quote.TotalNetCommission ?? 0) + (quote.TotalNetNonCommission ?? 0));

            //get Quote Total List & Net Values
            var _quote = new QuoteCalculateModel
            {
                //GrossMarginMarkUp = quote.CommissionPercentage,
                GrossMarginMarkUp = quote.GrossMarginMarkUp,
                DiscountPercentage = 0,
                DiscountPercentageSplit = 0,
                DiscountPercentageVRV = 0,
            ...
            var standardTotals = CalculateQuoteTotals(_quote);

            return standardTotals;
        }

        //TODO: Right a new function
        public QuoteCalculationModel CalculateTotals(QuoteCalculationRequest request)
        {
            ...
            //newCalc.GrossMarginMarkUp = request.CommissionPercentage; //  This is Buy/Sell-Commission Percentage (Gross profit percentage)
            newCalc.GrossMarginMarkUp = request.GrossMarginMarkUp; //  This is Buy/Sell-Commission Percentage (Gross profit percentage)
            ...
            if (request.IsGrossMargin)
            {
                ...
                if (request.GrossMarginMarkUp != 1)
                {
                    newCalc.TotalSell = newCalc.TotalNet / (1 - request.GrossMarginMarkUp);
                    newCalc.TotalSellSplit = newCalc.TotalNetSplit / (1 - request.GrossMarginMarkUp);
                    newCalc.TotalSellVRV = newCalc.TotalNetVRV / (1 - request.GrossMarginMarkUp);
                    newCalc.TotalSellUnitary = newCalc.TotalUnitary / (1 - request.GrossMarginMarkUp);
                    newCalc.TotalSellLCPackage = newCalc.TotalLCPackage / (1 - request.GrossMarginMarkUp);
                }
            }
            else // Mark up
            {
                ...
                newCalc.TotalSellSplit = newCalc.TotalNetSplit * (1 + request.GrossMarginMarkUp);
                newCalc.TotalSellVRV = newCalc.TotalNetVRV * (1 + request.GrossMarginMarkUp);
                newCalc.TotalSellUnitary = newCalc.TotalNetUnitary * (1 + request.GrossMarginMarkUp);
                newCalc.TotalSellLCPackage = newCalc.TotalNetLCPackage * (1 + request.GrossMarginMarkUp);
            }

            // Total Discount Amount

            newCalc.TotalCommissionAmount = newCalc.TotalSell - newCalc.TotalNet;// GrossMargin/Markup Amount
            ...
            newCalc.TotalDiscountPercentage = request.DiscountPercentage;
            ...
            if (newCalc.TotalDiscountPercentage == 0 && newCalc.TotalSell != 0)
            {
                ...
                newCalc.TotalDiscountPercentage = (1 - (newCalc.TotalPriceAfterDiscount / newCalc.TotalSell));
            }
            ...
            newCalc.TotalDiscountPercentageSplit = request.DiscountPercentageSplit;
            newCalc.TotalDiscountPercentageVRV = request.DiscountPercentageVRV;
            newCalc.TotalDiscountPercentageUnitary = request.DiscountPercentageUnitary;
            newCalc.TotalDiscountPercentageLCPackage = request.DiscountPercentageLCPackage;
        ...
        public QuoteCalculationModel CalculateQuoteTotals(QuoteCalculateModel _quote)
        {
            var newCalc = new QuoteCalculationModel();
            
            //Total List - Ok
            newCalc.TotalList = _quote.TotalList;
            newCalc.TotalListSplit = _quote.TotalListSplit;
            newCalc.TotalListVRV = _quote.TotalListVRV;
            newCalc.TotalListUnitary = _quote.TotalListUnitary;
            newCalc.TotalListLCPackage = _quote.TotalListLCPackage;

            newCalc.GrossMarginMarkUp = _quote.GrossMarginMarkUp; //  This is Buy/Sell-Commission Percentage (Gross profit percentage)

            //Total Net - OK
            newCalc.TotalNet = _quote.TotalNet - (_quote.TotalNet * _quote.DiscountPercentage);
            newCalc.TotalNetSplit = _quote.TotalNetSplit - (_quote.TotalNetSplit * _quote.DiscountPercentageSplit);
            newCalc.TotalNetVRV = _quote.TotalNetVRV - (_quote.TotalNetVRV * _quote.DiscountPercentageVRV);
            newCalc.TotalNetUnitary = _quote.TotalNetUnitary - (_quote.TotalNetUnitary * _quote.DiscountPercentageUnitary);
            newCalc.TotalNetLCPackage = _quote.TotalNetLCPackage - (_quote.TotalNetLCPackage * _quote.DiscountPercentageLCPackage);


            //Total Sell
            if (_quote.IsGrossMargin) // GrossMargin
            {
                newCalc.TotalSell = newCalc.TotalNet;
                newCalc.TotalSellSplit = newCalc.TotalNetSplit;
                newCalc.TotalSellVRV = newCalc.TotalNetVRV;
                ...
                if (_quote.GrossMarginMarkUp != 1)
                {
                    newCalc.TotalSell = newCalc.TotalNet / (1 - _quote.GrossMarginMarkUp);
                    newCalc.TotalSellSplit = newCalc.TotalNetSplit / (1 - _quote.GrossMarginMarkUp);
                    newCalc.TotalSellVRV = newCalc.TotalNetVRV / (1 - _quote.GrossMarginMarkUp);
                    newCalc.TotalSellUnitary = newCalc.TotalUnitary / (1 - _quote.GrossMarginMarkUp);
                    newCalc.TotalSellLCPackage = newCalc.TotalLCPackage / (1 - _quote.GrossMarginMarkUp);
                }
            }
            else // Mark up
            {
                ...
                newCalc.TotalSellSplit = newCalc.TotalNetSplit * (1 + _quote.GrossMarginMarkUp);
                newCalc.TotalSellVRV = newCalc.TotalNetVRV * (1 + _quote.GrossMarginMarkUp);
                newCalc.TotalSellUnitary = newCalc.TotalNetUnitary * (1 + _quote.GrossMarginMarkUp);
                newCalc.TotalSellLCPackage = newCalc.TotalNetLCPackage * (1 + _quote.GrossMarginMarkUp);
            }

            // Total Discount Amount

            newCalc.TotalCommissionAmount = newCalc.TotalSell - newCalc.TotalNet; // GrossMarginMarkup Amount

            newCalc.TotalDiscountAmount = _quote.TotalNet * _quote.DiscountPercentage;

            newCalc.TotalDiscountAmountSplit = _quote.TotalNetSplit * _quote.DiscountPercentageSplit;
            newCalc.TotalDiscountAmountVRV = _quote.TotalNetVRV * _quote.DiscountPercentageVRV;
            newCalc.TotalDiscountAmountUnitary = _quote.TotalNetUnitary * _quote.DiscountPercentageUnitary;
            newCalc.TotalDiscountAmountLCPackage = _quote.TotalNetLCPackage * _quote.DiscountPercentageLCPackage;

            //TODO:investigate what this is for.
            newCalc.TotalDiscountPercentage = _quote.DiscountPercentage;
            ...
            newCalc.TotalDiscountPercentageSplit = _quote.DiscountPercentageSplit;
            newCalc.TotalDiscountPercentageVRV = _quote.DiscountPercentageVRV;
            newCalc.TotalDiscountPercentageUnitary = _quote.DiscountPercentageUnitary;
            newCalc.TotalDiscountPercentageLCPackage = _quote.DiscountPercentageLCPackage;

            //Net Material Values - OK

            newCalc.NetMaterialValue = _quote.TotalNet - newCalc.TotalDiscountAmount;

            newCalc.NetMaterialValueSplit = _quote.TotalNetSplit - newCalc.TotalDiscountAmountSplit;
            newCalc.NetMaterialValueVRV = _quote.TotalNetVRV - newCalc.TotalDiscountAmountVRV;
            newCalc.NetMaterialValueUnitary = _quote.TotalNetUnitary - newCalc.TotalDiscountAmountUnitary;
            newCalc.NetMaterialValueLCPackage = _quote.TotalNetLCPackage - newCalc.TotalDiscountAmountLCPackage;
            ...
            if (newCalc.TotalList > 0)
            {
                newCalc.NetMultiplier = newCalc.NetMaterialValue / newCalc.TotalList;
            }

            if (newCalc.TotalListSplit > 0)
            {
                newCalc.NetMultiplierSplit = newCalc.NetMaterialValueSplit / newCalc.TotalListSplit;
            }

            if (newCalc.TotalListVRV > 0)
            {
                newCalc.NetMultiplierVRV = newCalc.NetMaterialValueVRV / newCalc.TotalListVRV;
            }

            if (newCalc.TotalListUnitary > 0)
            {
                newCalc.NetMultiplierUnitary = newCalc.NetMaterialValueUnitary / newCalc.TotalListUnitary;
            }

            if (newCalc.TotalListLCPackage > 0)
            {
                newCalc.NetMultiplierLCPackage = newCalc.NetMaterialValueLCPackage / newCalc.TotalListLCPackage;
            }
