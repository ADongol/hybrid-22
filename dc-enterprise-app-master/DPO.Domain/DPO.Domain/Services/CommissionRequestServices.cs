...
namespace DPO.Domain
{
    public partial class CommissionRequestServices : BaseServices
    {
        ...
        public CommissionRequestSendEmailModel GetCommissionRequestSendEmailModel(CommissionRequestModel req)
        {
            ...
            CommissionRequestSendEmailModel COMEmailModel = new CommissionRequestSendEmailModel();
            COMEmailModel.commissionRequest = req;
            COMEmailModel.AccountManagerEmail = business.AccountManagerEmail;
            COMEmailModel.AccountOwnerEmail = business.AccountOwnerEmail;

            //Add DVP/TSM/ASM/RSM/CSM Emails
            COMEmailModel.DVPEmail = GetEmailByUserId(business.DVP_UserId);
            COMEmailModel.TSMEmail = GetEmailByUserId(business.TSM_UserId);
            COMEmailModel.ASMEmail = GetEmailByUserId(business.ASM_UserId);
            COMEmailModel.RSMEmail = GetEmailByUserId(business.RSM_UserId);
            COMEmailModel.CSMEmail = GetEmailByUserId(business.CSM_UserId);

            return COMEmailModel;
        ...
        public MemoryStream GetCommissionRequestExportExcelFile(CommissionRequestModel model, bool showCostPricing)
        {
            ...
            if (showCostPricing == true)
            {
                ...
                {
                    r = worksheet.CreateRow(row++);
                    SetLabelValue(worksheet, r, 0, 1, "Requested Commission Percentage", 
                                string.Format("{0}%", Math.Round(model.RequestedCommissionPercentage*100, 2)));
                }
                ...
                if (!string.IsNullOrEmpty(model.ApprovedCommissionPercentage.ToString()))
                {
                    ...
                    SetLabelValue(worksheet, r, 0, 1, "Approved Commission Percentage", 
                                  string.Format("{0}%", Math.Round(model.ApprovedCommissionPercentage*100, 2)));
                }
        ...
        public ServiceResponse GetCommissionRequestModel(UserSessionModel admin, CommissionRequestModel req, CommissionCalculationModel calculationModel = null)
        {
            ...
            if (req.CommissionRequestId.HasValue)
            {
                var query = from commission in this.Db.QueryCommissionRequestsViewable(admin)
                            ...
                            select new CommissionRequestModel
                            {
                                ...
                                ApprovedCommissionVRV = (commission.RequestedCommissionVRV.HasValue ) ?
                                                        commission.RequestedCommissionVRV.Value : 0,
            ...
            model.hasOldCOM = (GetMostRecentCommissionRequestEntity(admin, (long)model.ProjectId, (long)model.QuoteId) != null);
        ...
        public CommissionRequest GetMostRecentCommissionRequestEntity(UserSessionModel admin, long projectId, long quoteId)
        {
            var query = from com in this.Db.QueryCommissionRequestsViewable(admin)
                        where com.ProjectId == projectId
                        select com;
            var mostRecentCOM = query.ToList().OrderBy(c => c.RequestedOn).FirstOrDefault();

            return mostRecentCOM;
        }

        public ServiceResponse GetMostRecentCommissionRequestModel(UserSessionModel admin, long projectId, long quoteId)
        {
            CommissionRequest com = GetMostRecentCommissionRequestEntity(admin, projectId, quoteId);
            this.Response = GetCommissionRequestModel(admin, new CommissionRequestModel { CommissionRequestId = com.CommissionRequestId, ProjectId = projectId, QuoteId = quoteId });
            return this.Response;
        }
        ...
        public void FinaliseModel(Messages messages, UserSessionModel admin, CommissionRequestModel model, CommissionCalculationModel calculationModel = null)
        {
            ...
            var projectService = new ProjectServices(this.Context);

            projectService.FinaliseModel(this.Response.Messages, admin, model.Project);
        ...
        public ServiceResponse PostModel(UserSessionModel admin, CommissionRequestModel model, CommissionCalculationModel calculationModel = null)
        {
            ...
            try
            {
                ...
                if (this.Response.IsOK)
                {
                    ...
                    var message = Utilities.SavePostedFile(model.CompetitorQuoteFile, Utilities.GetDARDirectory(model.QuoteId.Value), 512000);//TODO: Save to COMDirectory
                ...
                if (this.Response.IsOK)
                {
                    SaveToDatabase(model, entity, string.Format("Commission Request ", ""));
        ...
        public CommissionRequest ModelToEntity(UserSessionModel admin, CommissionRequestModel model)
        {
            ...
            if (admin.HasAccess(SystemAccessEnum.ApproveCommissionRequests))
            {
                entity.ResponseNotes = model.ResponseNotes;
            }
