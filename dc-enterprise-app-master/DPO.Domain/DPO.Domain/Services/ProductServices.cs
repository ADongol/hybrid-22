...
namespace DPO.Domain
{
    public partial class ProductServices : BaseServices
    {
        ...
        public ServiceResponse GetSellableProductByProductNumber(UserSessionModel user, ProductModel model)
        {
            var query = from e in this.Db.GetSellableProductsByProductNumber(user, model.ProductNumber)
                        select new ProductModel
                        {
                            ProductId = e.ProductId,
                            ProductNumber = e.ProductNumber,
        ...
        public long GetSellableProductId(UserSessionModel user, string productNumber)
        {
            var query = from e in this.Db.GetSellableProductsByProductNumber(user, productNumber)
                        select e.ProductId;
        ...
        public Product GetSellableProductbyProductNumber(UserSessionModel user, string productNumber)
        {
            var query = from entity in this.Db.GetSellableProductsByProductNumber(user, productNumber)
                        select entity;
        ...
        {
            if (this.Db.GetSellableProductsByProductNumber(user, productNumber).Any())
            {
                return true;
            }
            else
        ...
        public ServiceResponse GetAllProducts(SearchProduct search)
        {
            ...
            GetSpecifications(products,
               new string[] { "SEERNonDucted", "IEERNonDucted", "SCHENonDucted", "EERNonDucted", "HSPFNonDucted", "COP47NonDucted",
                                "SEERDucted", "IEERDucted", "EERDucted", "HSPFDucted" ,"COP47Ducted","AFUE",
                                "PipeSizeGas","PipeSizeLiquid","PowerPhase","PowerVoltage","PowerFrequency"});
        ...
        public ProductsModel SetActiveTab(ProductsModel model)
        {
            ...
            if (model.ProductFamilyId == (int)ProductFamilyEnum.MultiSplit || 
                model.ProductFamilyId == (int)ProductFamilyEnum.VRV || 
                model.ProductFamilyId == (int)ProductFamilyEnum.AlthermaSplit || 
                model.ProductFamilyId == (int)ProductFamilyEnum.AlthermaMonobloc)
            {
            ...
            if (model.ProductFamilyId == (int)ProductFamilyEnum.SkyAir ||
                model.ProductFamilyId == (int)ProductFamilyEnum.MiniSplit)
            {
                if (model.ProductModelTypeId == null)
                {
                    model.ProductModelTypeId = ProductModelTypeEnum.System;
                }
            }
            ...
            if (model.ProductFamilyId == (int)ProductFamilyEnum.UnitarySplitSystem || 
                ...
                model.ProductFamilyId == (int)ProductFamilyEnum.LightCommercialPackagedSystem)
            {
        ...
        public List<ProductModel> GetProductSubmittalData(UserSessionModel user, string[] productNumbers)
        {
            ...
            var query = from e in this.Db.GetSellableProductsByProductNumbers(user, productNumbers) select e;
        ...
        public ProductsModel ValidateLCSTPackagesModel(UserSessionModel user, LCSTPackagesModel packagesModel)
        {
            ...
            foreach (LCSTPackageModel pkg in packagesModel.Packages)
            {
                var baseModelProductId = GetSellableProductId(user, pkg.BaseModel);
                ...
                {
                    var accessoryProductId = GetSellableProductId(user, accessory.AccessoryModel);
