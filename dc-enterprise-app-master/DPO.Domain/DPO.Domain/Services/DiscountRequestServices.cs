...
namespace DPO.Domain
{
    public partial class DiscountRequestServices : BaseServices
    {
        ...
        public DiscountRequestSendEmailModel GetDiscountRequestSendEmailModel(DiscountRequestModel req)
        {
            ...
            DiscountRequestSendEmailModel DAREmailModel = new DiscountRequestSendEmailModel();
            DAREmailModel.discountRequest = req;
            DAREmailModel.AccountManagerEmail = business.AccountManagerEmail;
            DAREmailModel.AccountOwnerEmail = business.AccountOwnerEmail;

            //Add DVP/TSM/ASM/RSM/CSM Emails
            DAREmailModel.DVPEmail = GetEmailByUserId(business.DVP_UserId);
            DAREmailModel.TSMEmail = GetEmailByUserId(business.TSM_UserId);
            DAREmailModel.ASMEmail = GetEmailByUserId(business.ASM_UserId);
            DAREmailModel.RSMEmail = GetEmailByUserId(business.RSM_UserId);
            DAREmailModel.CSMEmail = GetEmailByUserId(business.CSM_UserId);

            return DAREmailModel;
        }
        ...
        public MemoryStream GetDiscountRequestExportExcelFile(DiscountRequestModel model, bool showCostPricing)
        {
            ...
            if (showCostPricing == true)
            {
                ...
                SetLabelValue(worksheet, r, 0, 1, "Rep/Distributor Gross Profit on Opportunity", model.StartUpCosts.ToString("0.00") + "/" + string.Format("{0:C}", model.GrossMarginMarkUp * model.StandardTotals.TotalNet));
                ...
                r = worksheet.CreateRow(row++);
                //SetLabelValue(worksheet, r, 0, 1, "Rep/Distributor revised gross profit on opportunity", (long)model.GrossMarginMarkUp);
                SetLabelValue(worksheet, r, 0, 1, "Rep/Distributor revised gross profit on opportunity", string.Format("{0}%", model.GrossMarginMarkUp*100));
                ...
                SetLabelValue(worksheet, r, 0, 1, "Revised Gross", string.Format("{0:C}", model.GrossMarginMarkUp * model.StandardTotals.NetMaterialValue));
            }
            ...
            if (req.DiscountRequestId.HasValue)
            {
                var query = from dis in this.Db.QueryDiscountRequestsViewable(admin)
                            ...
                            select new DiscountRequestModel
                            {
                                ...
                                GrossMarginMarkUp = (decimal)dis.GrossMarginMarkUp,
        ...
        public ServiceResponse GetDiscountRequest(UserSessionModel admin, DiscountRequestModel discountRequest) {
            DiscountRequestModel model = null;

            if (discountRequest.DiscountRequestId.HasValue)
            {
                var query = from dis in this.Db.QueryDiscountRequestsViewable(admin)
                            ...
                            where dis.DiscountRequestId == discountRequest.DiscountRequestId.Value
                            ...
                            select new DiscountRequestModel
                            {
                                ...
                                ShouldSendEmail = discountRequest.ShouldSendEmail,
            ...
            if (model == null)
            {
                ...
                {
                    DiscountRequestStatusTypeId = (byte)DiscountRequestStatusTypeEnum.NewRecord,
                    ProjectId = discountRequest.ProjectId,
                    QuoteId = discountRequest.QuoteId
                };
            }
            ...
            //Get Project Info
            GetProjectInfo(admin, model);

            QuoteServices quoteSvc = new QuoteServices();

            //GetQuoteInfo(admin, model);

            //Get Quote Info

            //TODO: May not needed. Calculation is done on client side, only need Reqested Discount % and Approved Discount %
            //Calculate Standard Totals
            model.StandardTotals = quoteSvc.CalculateStandardTotals(model.Quote);

            //Calculate Approved Totals
            model.ApprovedTotals = quoteSvc.CalculateApprovedTotals(model);

            model.hasOldDAR = (GetMostRecentDiscountRequestEntity(admin, (long)model.ProjectId, (long)model.QuoteId) != null);
        ...
        public DiscountRequest GetMostRecentDiscountRequestEntity(UserSessionModel admin, long projectId, long quoteId)
        {
            var query = from dar in this.Db.QueryDiscountRequestsViewable(admin)
                        where dar.ProjectId == projectId
                        select dar;
            var mostRecentDAR = query.ToList().OrderBy(d => d.RequestedOn).FirstOrDefault();
            
            return mostRecentDAR;
        }

        public ServiceResponse GetMostRecentDiscountRequestModel(UserSessionModel admin, long projectId, long quoteId) {
            DiscountRequest dar = GetMostRecentDiscountRequestEntity(admin, projectId, quoteId);
            this.Response = GetDiscountRequest(admin, new DiscountRequestModel { DiscountRequestId = dar.DiscountRequestId, ProjectId = projectId, QuoteId = quoteId });
            return this.Response;
        }
        ...
        public void FinaliseModel(Messages messages, UserSessionModel admin, DiscountRequestModel model)
        {
            ...
            //Calculat Totals. If ... else ... are doing the same thing ...
            if (model.ApprovedDiscount > 0)
            {
                model.ApprovedTotals = service.CalculateTotalDiscountsApproved(model.Quote, model.ApprovedDiscount,
                    model.ApprovedDiscountSplit, model.ApprovedDiscountVRV, model.ApprovedDiscountUnitary, model.ApprovedDiscountLCPackage, model.GrossMarginMarkUp);
            }
            else
            {
                model.ApprovedTotals = service.CalculateTotalDiscountsApproved(model.Quote, model.RequestedDiscount,
                    model.RequestedDiscountSplit, model.RequestedDiscountVRV, model.RequestedDiscountUnitary, model.RequestedDiscountLCPackage, model.GrossMarginMarkUp);
            }
        ...
        public void GetProjectInfo(UserSessionModel admin, DiscountRequestModel model) {
                                   ...
                                   select new ProjectModel
                                   {
                                       ...
                                       ActiveQuoteSummary = new QuoteListModel
                                       {
                                           ...
                                           TotalNetLCPackage = (quote == null) ? 0 :
                                                             (quote.TotalNetLCPackage.HasValue) ? quote.TotalNetLCPackage.Value : 0,
                                           IsGrossMargin = (quote == null) ? false : quote.IsGrossMargin,
                                           GrossMarginMarkUp = (quote == null) ? 0 : quote.GrossMarginMarkUp,
                                           TotalFreight = (quote == null) ? 0 : quote.TotalFreight,
                                           DiscountPercentage = (quote == null) ? 0 : quote.DiscountPercentage,
                                           CommissionPercentage = (quote == null) ? 0 : quote.CommissionPercentage,
                                           Revision = (quote == null) ? 0 : quote.Revision
        ...
        public void GetQuoteInfo(UserSessionModel admin, DiscountRequestModel model)
        {
            if (model.ProjectId.HasValue && model.QuoteId.HasValue) {
                QuoteModel quoteModel = QuoteServices.GetQuoteModel(admin, model.ProjectId, model.QuoteId).Model as QuoteModel;
                model.GrossMarginMarkUp = quoteModel.GrossMarginMarkUp;
            }
        }
        ...
        public ServiceResponse PostModel(UserSessionModel admin, DiscountRequestModel model)
        {
            ...
            try
            {
                ...
                if (this.Response.IsOK)
                {
                    SaveToDatabase(model, entity, string.Format("Discount Request ", ""));
                }
        ...
        public ServiceResponse PostDiscountRequest(UserSessionModel admin, DiscountRequestModel model)
        {
            this.Db.ReadOnly = false;
        ...
        private DiscountRequest ModelToEntity(UserSessionModel admin, DiscountRequestModel model)
        {
            ...
            entity.GrossMarginMarkUp = model.GrossMarginMarkUp;
