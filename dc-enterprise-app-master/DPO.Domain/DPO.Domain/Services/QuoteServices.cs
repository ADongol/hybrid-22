    ...
    public partial class QuoteServices : BaseServices
    {
        ...
        public ServiceResponse GetQuoteModel(UserSessionModel admin, Quote quote)
        {
           // Log.Info("enter GetQuoteModel");
            return GetQuoteModel(admin, (quote == null) ? 
                null : (long?)quote.ProjectId, (quote == null) ? null : (long?)quote.QuoteId);
        }
        ...
        public ServiceResponse GetQuoteModel(UserSessionModel admin, long? projectId, long? quoteId)
        {
            ...
            if (quoteId.HasValue)
            {
                var query = from q in this.Db.QueryQuoteViewableByQuoteId(admin, quoteId.Value)
                            ...
                            //let rd = discount == null? DateTime.MinValue: discount.RequestedOn
                            //orderby rd descending
                            select new QuoteModel
                            {
                                ...
                                GrossMarginMarkUp = q.GrossMarginMarkUp,
                                ...
                                CommissionRequestId = (q.CommissionRequestId == null) ? 
                                                    commission.CommissionRequestId : q.CommissionRequestId,
                                ...
                                CommissionRequestStatusTypeId = com.OrderByDescending(c => c.RequestedOn).FirstOrDefault() == null ? (byte)0: com.OrderByDescending(c => c.RequestedOn).FirstOrDefault().CommissionRequestStatusTypeId.Value,
                                //DiscountRequestStatusTypeId = (discount != null) ? discount.DiscountRequestStatusTypeId : (byte)0,
                                DiscountRequestStatusTypeId = dis.OrderByDescending(d => d.RequestedOn).FirstOrDefault() == null ? (byte)0: dis.OrderByDescending(d => d.RequestedOn).FirstOrDefault().DiscountRequestStatusTypeId,
            ...
            model.IsDeleted = CheckIfQuoteIsDeleted(quoteId); //check if quote is deleted
        ...
        public ServiceResponse GetQuoteItemListModel(UserSessionModel admin, SearchQuoteItem search)
        {
            ...
            {
                //this.Response.Model = query.ToList();
                this.Response.Model = query.OrderBy(i => i.Rank).ToList();
            }
            catch (Exception ex)
            {
            ...
            if (loadQuoteItems)
            {
                ...
                foreach (var item in model.Items) {
                    ...
                    if (item.LineItemTypeId != (byte)LineItemTypeEnum.Configured) {
                        if (item.ProductStatusTypeId == (int)ProductStatusTypeEnum.Obsolete && item.InventoryStatusId == (int)ProductInventoryStatusTypeEnum.NotAvailable)
                        {
                            model.HasObsoleteAndUnavailableProduct = true;
                        }
                        if (item.ProductStatusTypeId == (int)ProductStatusTypeEnum.Obsolete)
                        {
        ...
        public ServiceResponse GetQuoteItems(UserSessionModel admin, long? quoteId)
        {
            ...
            if (quoteId.HasValue)
            {
                ...
                var model = new List<QuoteItemModel>();
        ...
        {
            var quoteOptions = new QuoteOptionsModel();
        ...
        public bool CheckIfQuoteIsDeleted(long? quoteId)
        {
            var isQuoteDeleted = Db.Context.Quotes.Any(q => q.QuoteId == quoteId && q.Deleted == true);
        ...
        public ServiceResponse ImportProductsFromCSV(UserSessionModel user, CsvReader data, QuoteModel model)
        {
            ...
            if (itemsDict.Count > 0)
            {
                ...
                var productLookup = Db.SellableProductByProductNumbers(user, productNumberList).Distinct().ToArray().ToDictionary(p => p.ProductNumber, p => p.ProductId);
        ...
        public ServiceResponse ImportProductsFromXml(UserSessionModel user, HttpPostedFileBase file, QuoteModel model)
        {
            ...
            if (itemsDict.Count > 0)
            {
                ...
                var results = Db.SellableProductByProductNumbers(user, productNumberList)
        ...
        public ServiceResponse AddProductsToQuote(UserSessionModel user, QuoteModel model, ProductsModel productsModel)
        {
           // Log.InfoFormat("Enter AddProductsToQuote for {0}, {1} ", model.GetType(), productsModel.GetType());
            ...
            if (model != null)
            {
                ...
                var QuoteItemsList = new List<QuoteItem>();
        ...
        public List<QuoteItem> BuildQuoteItemList(UserSessionModel user, long quoteId, LCSTPackagesModel packagesModel)
        {
            ...
            for (int i = 0; i < packagesModel.Packages.Count; i++)
            {
                ...
                {
                    var baseModel = productServices.GetSellableProductbyProductNumber(user, package.BaseModel);

                    if (package.Accessories.Count() == 0)
                    {
                        //If LC product has only baseModel, add baseModel as regular item
                        var quoteItem = new QuoteItem();
                        quoteItem.ProductId = baseModel.ProductId;
                        quoteItem.Quantity = 1;
                        quoteItem.ListPrice = baseModel.ListPrice;
                        quoteItem.LineItemTypeId = (byte?)LineItemTypeEnum.Standard;
                        quoteItem.ConfigType = package.ConfigType;
                        quoteItem.Tags = package.SystemId;
                        QuoteItemsList.Add(quoteItem);
                    }
                    else if (baseModel != null && baseModel.ProductId > 0)
                    {
                        ...
                        configuredQuoteItem.ModelId = package.ModelId;
                        configuredQuoteItem.EfficiencyTypeId = (package.Efficiency == "High") ? (byte?)EfficiencyTypeEnum.High : (byte?)EfficiencyTypeEnum.Standard;
                        ...
                        {
                            var product = productServices.GetSellableProductbyProductNumber(user, accessory.AccessoryModel);

                            if (product != null && accessory.AccessoryType == "Y") // Y means FactoryInstalled
                            {
                            ...
                            else if (product != null && accessory.AccessoryType == "N") // N means Standard/ Field Installed
                            {
        ...
        private void AdjustQuoteItems(UserSessionModel user, QuoteModel model, List<QuoteItem> QuoteItemUpdates, bool isAdd, Quote entity = null)
        {
            ...
            if (quote != null && QuoteItemUpdates != null && QuoteItemUpdates.Count > 0)
            {
                ...
                var maxRank = quoteItems.Max(i => i.Rank);
                ...
                QuoteItemUpdates.ForEach(import =>
                {
                    ...
                    if (import.LineItemTypeId == (byte?)LineItemTypeEnum.Configured) // Configured QuoteItem
                    {
                        ...
                        if (quoteItem == null && import.ProductId > 0)// new quoteItem
                        {
                            ...
                            quoteItem.ModelId = import.ModelId;
                            quoteItem.EfficiencyTypeId = import.EfficiencyTypeId;
                            quoteItem.LineItemTypeId = import.LineItemTypeId;
                            quoteItem.ConfigType = import.ConfigType;
                            quoteItem.Rank = maxRank + 1;
                            maxRank++;
                        ...
                        } else if (quoteItem != null && import != null) {
                            quoteItem.Rank = import.Rank;
                        }
                    }
                    else// Regular QuoteItem
                    {
                        quoteItem = (import.QuoteItemId == 0) ?
                        ...
                        else if(quoteItem != null && import != null) // existing quoteItem (except removed item). When item has been removed from quote, quoteItem == null && import.productId == null
                        { 
                            quoteItem.Rank = import.Rank;
                        }
                    }
        ...
        public decimal CalculateConfiguredItemPrice(UserSessionModel user, LCSTPackageModel configuredItem)
        {
            ...
            var baseModel = productServices.GetSellableProductbyProductNumber(user, configuredItem.BaseModel);
            if (baseModel != null)
            {
            ...
            foreach (var accessory in configuredItem.Accessories)
            {
                ...
                {
                    var item = productServices.GetSellableProductbyProductNumber(user, accessory.AccessoryModel);
                    if (item != null)
                    {
        ...
        public bool CanEditQuote(UserSessionModel user, QuoteModel quoteModel)
        {
            ...
            else if (quoteModel.IsTransferred || (quoteModel.HasDAR && (quoteModel.DiscountRequestStatusTypeId == (byte)DiscountRequestStatusTypeEnum.Pending || quoteModel.DiscountRequestStatusTypeId == (byte)DiscountRequestStatusTypeEnum.Approved)))
            {
                return false;
            }
        ...
        #region CheckProductWithNoClassCode
        ...
        public void ConvertPdfFromBase64String(string base64Str)
        {
            ...
            var nameFile = "Test Submittal" + ".pdf";
            var subPath = grandParent + "/CustomerDataFiles/Submittals";
            var exists = Directory.Exists(subPath);

            if (!exists)
                Directory.CreateDirectory(subPath);

            var filePath = grandParent + "/CustomerDataFiles/Submittals/" + nameFile;
        ...
        public void SaveLCSubmittalData(LCSTPackagesModel packagesModel)
        {
            ...
            {
                var filePath = SubmittalDirectory + package.Model + ".pdf";
                using (FileStream stream = System.IO.File.Create(filePath))
                {
