    ...
    public class AccountServices : BaseServices
    {
        ...
        public ServiceResponse GetDefaultPageUrl(UserSessionModel user, string selectedLink = "")
        {
            ...
            //}
                ...
                && (selectedLink.ToLower().Contains("/v3/#/projects")
                    || selectedLink.ToLower().Contains("projectdashboard/overview")
                    || String.IsNullOrEmpty(selectedLink)))
            {
                url = "/ProjectDashboard/Tools";
        ...
        public ServiceResponse Login(UserLoginModel model)
        {
            ...
            //commented out because of new requirement of letting user log in with provisional accesses
            //if (!user.Approved)
            //{
            ...
            if (user.Rejected)
            {
                this.Response.Messages.AddError("Email", ResourceModelUser.MU025);
                return this.Response;
            }

            //When user is in pending status, allow log in even if business is not enabled 
            // like when user choses Other option while registering.
            if (!user.Business.Enabled)
            {
                //But don't let user in if business gets disabled after user was already approved
                if (user.Approved)
                {
