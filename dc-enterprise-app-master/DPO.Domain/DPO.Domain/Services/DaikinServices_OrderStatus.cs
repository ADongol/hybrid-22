    ...
    public partial class DaikinServices : BaseServices
    {
        ...
        private void ProcessOrdersInSubmittedStatus(ERPClient erpClient)
        {
            ...
            if (orderList != null && orderList.Count() > 0)
            {
                ...
                var orderToUpdateList = Db.Orders?.Where(x => x.OrderStatusTypeId == 2).ToList();
                ...
                if (orderToUpdateList != null && orderToUpdateList.Count() > 0)
                {
                    foreach (var order in orderList.ToList())
                    {
                        ...
                        if (orderToUpdate != null)
                        {
                            ...
                            orderToUpdate.WebServiceImportStatus = "FromSubmittedToAwaitingCSR";

                            UpdateOrderStatusInDC(orderToUpdate);
                        }
                    }
                }
            }
        ...
        private void ProcessOrdersInAwaitingCSRStatus(ERPClient erpClient)
        {
            ...
            if (orderList != null && orderList.Count() > 0)
            {
                ...
                var orderToUpdateList = Db.Orders?.Where(x => x.OrderStatusTypeId == 3 || x.OrderStatusTypeId == 4).ToList();
                    ...
                    foreach (var order in groupedByOrderList.ToList().Where(x => orderToUpdateList.Any(y => y.QuoteId == x.QuoteID)))
                    {
                        ...
                        if (orderToUpdate != null)
                        {
                            ...
                            if (order.OrderDate == DateTime.MinValue)
                                orderToUpdate.ERPOrderDate = null;
                            else
                            ...
                            orderToUpdate.WebServiceImportStatus = "FromAwaitingCSRtoInProcess";
        ...
        private void ProcessOrdersInProcessStatus(ERPClient erpClient)
        {
            ...
            var orderToUpdateList = Db.Orders?.Where(x => x.OrderStatusTypeId == 5).ToList();
            ...
            if (orderToUpdateList != null && orderToUpdateList.Count() > 0)
            {
                foreach (var item in orderToUpdateList)
                {
                    ...
                    Console.WriteLine($"Sent request to ERP for Order Number - { item.ERPOrderNumber} to check if the order is processed");
                    ...
                    if (result != null && result.ProjectID != 0)
                    {
                        ...
                        if (orderToUpdate != null)
                        {
                            ...
                            orderToUpdate.WebServiceImportStatus = "FromInProcessToPicked";
                    ...
                    {
                        Console.WriteLine($"No match found in Mapics for - { item.ERPOrderNumber} to confirm it's in-process status");
                    }
                }
            }
        }
        ...
        private void ProcessOrdersInPickedStatus(ERPClient erpClient)
        {
            ...
            var orderToUpdateList = Db.Orders?.Where(x => x.OrderStatusTypeId == 6).ToList();
            ...
            if (orderToUpdateList != null && orderToUpdateList.Count() > 0)
            {
                foreach (var item in orderToUpdateList)
                {
                    ...
                    Console.WriteLine($"Sent request to ERP for Order Number - { item.ERPOrderNumber} to check if the order is picked");
                    ...
                    if (result != null && result.QuoteID != 0)
                    {
                        ...
                        if (orderToUpdate != null)
                        {
                            ...
                            if (result.InvoiceDate == DateTime.MinValue)
                                orderToUpdate.ERPInvoiceDate = null;
                            else
                                orderToUpdate.ERPInvoiceDate = result.InvoiceDate;

                            if (result.ShipmentDate == DateTime.MinValue)
                                orderToUpdate.ERPShipDate = null;
                            else
                            ...
                            var quote = Db.Quotes?.FirstOrDefault(x => x.QuoteId == orderToUpdate.QuoteId); //Get matching quote
                            if (quote != null)
                            {
                                Console.WriteLine($"Project match found for  {quote.ProjectId} in DC");
        ...
        private void UpdateProjectsToOpenOrderStatus(long projectId, OrderResponse orderData)
        {
            ...
            if (projectToUpdate != null)
            {
                ...
                projectToUpdate.WebServiceImportStatus = "FromAwaitingCSRToInProcess";
                projectToUpdate.ModifyOn = DateTime.UtcNow;
                projectToUpdate.ModifyBy = 1111111111;

                Db.Context.SaveChanges();
                Console.WriteLine($"Updated project - {projectToUpdate.ProjectId}");
            }
        }
        ...
        private void UpdateProjectsToShippedStatus(long projectId, ERPInvoiceInfo invoiceData)
        {
            ...
            if (projectToUpdate != null)
            {
                ...
                projectToUpdate.Deleted = false;
                projectToUpdate.WebServiceImportStatus = "FromInProcessToPicked";
                projectToUpdate.ModifyOn = DateTime.UtcNow;
                projectToUpdate.ModifyBy = 1111111111;
        ...
        private void UpdateOrderStatusInDC(Order orderToUpdate)
        {
            orderToUpdate.ModifyOn = DateTime.UtcNow;
            orderToUpdate.UpdatedByUserId = 1111111111;
