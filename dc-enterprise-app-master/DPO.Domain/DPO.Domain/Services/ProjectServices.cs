...
namespace DPO.Domain
{
    public partial class ProjectServices : BaseServices
    {
        ...
        private ProjectStrategicTypesServices projectStrategicTypesService;
            ...
            : base()
        {
            ...
            projectStrategicTypesService = new ProjectStrategicTypesServices(this.Context);
        }
        ...
        public ServiceResponse GetProjectModel(UserSessionModel admin, long? projectId)
        {
            ...
            if (projectId.HasValue)
            {
                var query = from project in this.Db.QueryProjectViewableByProjectId(admin, projectId)
                            ...
                            select new ProjectModel
                            {
                                ...
                                IsStrategicProject = (project.IsStrategicProject == null) ? false : (bool)project.IsStrategicProject
                                //IsStrategicProject = project.IsStrategicProject
                            };
            ...
            if (model.ProjectId.HasValue) {
                model.SelectedStrategicTypes = projectStrategicTypesService.GetStrategicTypesByProjectId(model.ProjectId.Value)
                                                .Select(t => t.StrategicTypeId).ToList();
            }
        ...
        public ServiceResponse GetAllProjects(UserSessionModel admin, ProjectsGridViewModel model, ProjectsGridQueryInfo queryInfo)
        {
            ...
            model.Items.ForEach(p =>
            {
                if (p.IsStrategicProject)
                {
                    p.SelectedStrategicTypes = projectStrategicTypesService.GetStrategicTypesByProjectId(p.ProjectId.Value)
                                                        .Select(t => t.StrategicTypeId).ToList();

                    var htmlService = new HtmlServices(this.Context);
                    p.StrategicTypes = htmlService.DropDownModelStrategicTypes((p.SelectedStrategicTypes.Count() == 0) ? null : p.SelectedStrategicTypes);
                }
        ...
        private IQueryable<ProjectViewModel> Filter(UserSessionModel currentUser, IQueryable<ProjectViewModel> query, QueryInfo queryInfo)
        {
            ...
            if (!currentUser.HasAccess(SystemAccessEnum.RequestCommission) && !currentUser.HasAccess(SystemAccessEnum.ViewRequestedCommission) && !currentUser.HasAccess(SystemAccessEnum.ApproveCommissionRequests))
            {
        ...
        private IQueryable<ProjectViewModel> ProcessSingleFilter(IQueryable<ProjectViewModel> query, string field, string op, string val)
        {
            switch (field)
            {
                ...
                case "isCommission":
                    ...
                    break;
                case "isStrategicProject":
                    if (op == "eq")
                    {
                        var value = Boolean.Parse(val);
                        query = query.Where(p => p.IsStrategicProject == value);
                    }
                    break;
        ...
        private IQueryable<ProjectViewModel> Sort(IQueryable<ProjectViewModel> query, QueryInfo queryInfo)
        {
            if (queryInfo.Sort.Count() > 0)
            {
                ...
                switch (sortcolumn)
                {
                    ...
                    case "isstrategicproject":
                        query = (desc) ? query.OrderBy(s => s.IsStrategicProject) : query.OrderByDescending(s => s.IsStrategicProject);
                        break;

                    case "pricingstrategy":
                        query = (desc) ? query.OrderBy(s => s.IsCommission) : query.OrderByDescending(s => s.IsCommission);
                        break;
        ...
        public Project ModelToEntity(UserSessionModel admin, ProjectModel model)
        {
            ...
            if (model.SelectedStrategicTypes.Count < 1)
            {
                model.IsStrategicProject = false;
            }

            entity.IsStrategicProject = model.IsStrategicProject;

            

            model.ProjectId = entity.ProjectId;

            UpdateProjectStrategicTypes(admin, model);
        ...
        public void UpdateProjectStrategicTypes(UserSessionModel admin, ProjectModel model) {
            if (model.IsStrategicProject && model.SelectedStrategicTypes.Count > 0)
            {

                List<long> currentStrategicTypes = projectStrategicTypesService.GetStrategicTypesByProjectId(model.ProjectId.Value)
                                                    .Select(t => t.StrategicTypeId)
                                                    .ToList();

                if (!model.SelectedStrategicTypes.Equals(currentStrategicTypes))
                {
                    //remove old types
                    projectStrategicTypesService.RemoveStrategicTypesByProjectId(model.ProjectId.Value);
                    //add new types
                    foreach (var strategicTypeId in model.SelectedStrategicTypes)
                    {
                        projectStrategicTypesService.ProjectStrategicTypeInsert(admin, model.ProjectId.Value, strategicTypeId);
                    }
                }
            }
            else
            {
                //remove old types if exists
                projectStrategicTypesService.RemoveStrategicTypesByProjectId(model.ProjectId.Value);
            }
        }
        ...
        public ServiceResponse ValidateProjectLocationAddress(ProjectModel model)
        {
            ...
            var AddressDt = new DataTable("ShippingAddressSuggestion");
            var AddressLine1 = new DataColumn("AddressLine1", typeof(string));
            var AddressLine2 = new DataColumn("AddressLine2", typeof(string));
            var State = new DataColumn("State", typeof(string));
            var ZipCode = new DataColumn("ZipCode", typeof(string));
            var City = new DataColumn("City", typeof(string));
        ...
        public ServiceResponse ValidateSellerAddress(ProjectModel model)
        {
            ...
            var AddressDt = new DataTable("SellerAddressSuggestion");
            var AddressLine1 = new DataColumn("AddressLine1", typeof(string));
            var AddressLine2 = new DataColumn("AddressLine2", typeof(string));
            var State = new DataColumn("State", typeof(string));
            var ZipCode = new DataColumn("ZipCode", typeof(string));
        ...
        public ServiceResponse ValidateEngineerAddress(ProjectModel model)
        {
            ...
            var AddressDt = new DataTable("EngineerAddressSuggestion");
            var AddressLine1 = new DataColumn("AddressLine1", typeof(string));
            var AddressLine2 = new DataColumn("AddressLine2", typeof(string));
            var State = new DataColumn("State", typeof(string));
            var ZipCode = new DataColumn("ZipCode", typeof(string));
        ...
        public void FinaliseModel(Messages messages, UserSessionModel admin, ProjectModel model)
        {
            ...
            //model.StrategicTypes = service.DropDownModelStrategicTypes(null);
            model.StrategicTypes = service.DropDownModelStrategicTypes((model == null) ? null : model.SelectedStrategicTypes);
        }
