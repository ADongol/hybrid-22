...
namespace DPO.Domain
{
    public partial class CityCMSServices : BaseServices
    {
        ...
        #region Maintenance Config
        public List<MaintenancePageModel> GetMaintenanceConfigs()
        {
            return (from config in this.Context.DCMaintenanceConfigs?.OrderByDescending(x => x.StartTime)
        ...
        public MaintenancePageModel GetMaintenanceConfigEdit(int configId)
        {
            return (from config in this.Context.DCMaintenanceConfigs
                    where config.MaintenanceId == configId
                    select new MaintenancePageModel
                    {
                        MaintenanceId = config.MaintenanceId,
                        MaintenanceText = config.MaintenanceText,
        ...
        public void GetMaintenanceConfigDelete(int configId)
        {
            var entity = this.Context.DCMaintenanceConfigs?.Where(x => x.MaintenanceId == configId)
                               .FirstOrDefault();

            this.Context.DCMaintenanceConfigs.Remove(entity);
            this.Context.SaveChanges();
        }

        public void SaveMaintenaceConfig(MaintenancePageModel model)
        {
            this.Db.ReadOnly = false;

            if (model.MaintenanceId == 0)
            {
                var entity = new DCMaintenanceConfig
                {
                    MaintenanceText = model.MaintenanceText,
                    StartTime = model.StartTime,
                    EndTime = model.EndTime,
                    ModifyBy = 206016535569891328,
                    ModifyOn = DateTime.Now,
                    Timestamp = DateTime.Now
                };

                this.Context.DCMaintenanceConfigs.Add(entity);
                this.Context.SaveChanges();
            }
            else
            {
                var entity = this.Db.QueryMaintenanceConfig(model.MaintenanceId); 
                entity.MaintenanceText = model.MaintenanceText;
